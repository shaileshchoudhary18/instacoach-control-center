...
const tabs = [
  { name: "Promotions Control", href: "/operations/promotions-control" },
];
    ...
    <div className="min-h-full flex flex-col">
      <div className="bg-[#050505]/80 backdrop-blur-xl border-b border-white/5 sticky top-0 z-30 pt-6 px-8 flex flex-col gap-4">
        <div>
          ...
          <p className="text-sm font-medium text-slate-400 mt-1">Manage marketing, campaigns and client operations.</p>
        </div>
          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link key={tab.name} href={tab.href} className="relative pb-3">
                ...
                {isActive && (
                  <motion.div layoutId="operations-tab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-indigo-500 rounded-t-full shadow-[0_0_8px_rgba(99,102,241,0.8)]" />
                )}
              </Link>
            );
          })}
